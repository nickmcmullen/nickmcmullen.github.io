...
for(i in 1:length(Florida$Poll)){
  if(Florida$RMargin[i] > 0){
    ...
    posterior[i,2] <- 1- postMeanR #(priorAlpha[2]/alphaSum)
  ...
  if(Florida$RMargin[i] < 0){
    ...
    posterior[i,1] <- 1-postMeanD #(priorAlpha[1]/alphaSum)
    #posterior[i,3] <- 1 - (posterior[i,1] + posterior[i,2])
    #Update the prior for sequential analysis
    priorAlpha[2] <- alphaD
  }
...
ggplot(posterior, aes(x=Date, y=pWinScott))+
  ...
  scale_colour_manual("", breaks=c("Scott (R)", "Nelson (D)"), values=c("Scott (R)"="red","Nelson (D)"="blue")) + 
  lims(y=c(.25,.75))
